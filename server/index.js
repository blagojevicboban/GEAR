...
app.get('/api/models', async (req, res) => {
    try {
        ...
        // Migration Check for Analytics
        try {
            await pool.query('SELECT 1 FROM analytics_logs LIMIT 1');
        } catch (e) {
...
// --- Analytics API (Heatmaps) ---

// 1. Log Telemetry (Batch)
...
// 2. Get Heatmap Data (Aggregated Point Cloud)
app.get('/api/analytics/heatmap/:modelId', async (req, res) => {
    const { modelId } = req.params;
    try {
        // Fetch raw points. In a real app, we might cluster this on the server.
        // For now, return raw targets (where they looked).
