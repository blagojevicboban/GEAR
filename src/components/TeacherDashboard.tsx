...
const TeacherDashboard: React.FC<{ currentUser: any; models?: VETModel[] }> = ({
    currentUser,
    models = [],
}) => {
    ...
    const [selectedModelForHeatmap, setSelectedModelForHeatmap] =
        useState<VETModel | null>(null);
                ...
                <>
                    ...
                    <div className="relative mb-6">
                        ...
                        <input
                            type="text"
                            placeholder={t(
                                'teacher.dashboard.search_placeholder'
                            )}
                            value={searchTerm}
                                ...
                                <thead>
                                    <tr className="bg-slate-950 text-slate-400 text-xs uppercase tracking-wider">
                                        <th className="p-4 font-semibold">
                                            {t(
                                                'teacher.dashboard.table.student'
                                            )}
                                        </th>
                                        <th className="p-4 font-semibold">
                                            {t(
                                                'teacher.dashboard.table.lesson'
                                            )}
                                        </th>
                                        <th className="p-4 font-semibold">
                                            {t(
                                                'teacher.dashboard.table.status'
                                            )}
                                        </th>
                                        <th className="p-4 font-semibold">
                                            {t('teacher.dashboard.table.score')}
                                        </th>
                                        <th className="p-4 font-semibold">
                                            {t(
                                                'teacher.dashboard.table.started'
                                            )}
                                        </th>
                                        <th className="p-4 font-semibold">
                                            {t(
                                                'teacher.dashboard.table.completed'
                                            )}
                                        </th>
                                    </tr>
                                </thead>
                ...
                <div className="animate-in fade-in slide-in-from-bottom-4">
                    {!selectedModelForHeatmap ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {models
                                .filter(
                                    (m) =>
                                        ...
                                        m.uploadedBy === currentUser.username
                                )
                                .map((model) => (
                                    <div
                                        key={model.id}
                                        onClick={() =>
                                            setSelectedModelForHeatmap(model)
                                        }
                            ...
                            {models.length === 0 && (
                                <div className="text-slate-500 col-span-3 text-center">
                                    No models available for analytics.
                                </div>
                            )}
                        </div>
                    ) : (
                        <div>
                            ...
                            <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
                                ...
                                <p className="text-sm text-slate-400 mb-6 max-w-2xl bg-slate-950 p-4 rounded-lg border border-slate-800">
                                    ...
                                    Red dots represent points on the model where
                                    students spent time looking or hovering
                                    during VR/3D sessions. Density indicates
                                    higher engagement.
                                </p>
                                <HeatmapViewer
                                    model={selectedModelForHeatmap}
                                />
                            </div>
                        </div>
                    )}
