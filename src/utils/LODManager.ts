...
export class LODManager {
    static getTier(): PerformanceTier {
        ...
        const isNvidia = /NVIDIA/i.test(rendererString) || /GeForce/i.test(rendererString);
        const isRadeon = /Radeon/i.test(rendererString);

        // Tier Logic
        if (isMobile) {
            if (cores <= 4 || textIncludes(rendererString, ['Mali-G5', 'Adreno 5', 'PowerVR'])) {
        ...
        // Desktop / Laptop
        if (isNvidia || (isRadeon && !textIncludes(rendererString, ['Integrated']))) {
            return PerformanceTier.HIGH;
        }

        if (isIntel || textIncludes(rendererString, ['Integrated', 'UHD', 'Iris'])) {
