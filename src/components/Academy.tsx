...
const Academy: React.FC<AcademyProps> = ({ currentUser }) => {
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            ...
            newVids[activeCategory] = newVids[activeCategory].filter(
                (v: any) => v.id !== id
            );
            setVideos(newVids);
        } catch (e) {
    ...
    const handleSave = async () => {
        if (!newTitle || !newUrl)
            return alert(t('academy.form.title') + ' & URL required');
                    ...
                    <h3 className="font-bold text-white mb-4">
                        {editingId
                            ...
                            : t('academy.form.add_new', {
                                  category: t(`academy.${activeCategory}`),
                              })}
                    </h3>
