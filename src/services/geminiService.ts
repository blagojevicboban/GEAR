 ...
 */
export const analyzeModelDescription = async (name: string, description: string) => {
  try {
    ...
    const response = await ai.models.generateContent({
      ...
      config: {
        systemInstruction: "You are an expert VET curriculum designer. Provide a JSON array of objects with taskName, description, and difficulty. Ensure the response is strictly valid JSON.",
 ...
 */
export const generateOptimizationSuggestions = async (fileSize: number, sector: string) => {
  try {
    const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
    ...
    const response = await ai.models.generateContent({
      ...
      config: {
        systemInstruction: "You are a WebXR performance expert. Respond with clear, concise optimization tips as plain text list.",
      }
    });
    
    const text = response.text;
    return text || "1. Use Draco mesh compression.\n2. Limit texture resolution to 2K.\n3. Reduce draw calls by merging static meshes.";
  } catch (error) {
    const errorMsg = error instanceof Error ? error.message : String(error);
