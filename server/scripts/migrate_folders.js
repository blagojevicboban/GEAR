...
const migrate = async () => {
    ...
    try {
        const [models] = await connection.query(
            'SELECT id, name, modelUrl, thumbnailUrl FROM models'
        );
        ...
        for (const model of models) {
            ...
            const processFile = (fileUrl, isThumbnail = false) => {
                ...
                if (
                    ...
                    !fileUrl.includes(process.env.CLIENT_URL || 'localhost')
                ) {
                ...
                const relativePath = pathWithoutHash.replace(
                    '/api/uploads/',
                    ''
                );
                ...
                if (
                    parts.length > 1 &&
                    parentFolderName.startsWith(expectedPrefix)
                ) {
                ...
                try {
                    ...
                    console.log(
                        `Migrated ${fileName} -> ${newFolderName}/${fileName}`
                    );
            ...
            if (
                ...
                if (
                    currentModelUrl &&
                    currentModelUrl.startsWith('/api/uploads/')
                ) {
                    const modelRel = currentModelUrl.replace(
                        '/api/uploads/',
                        ''
                    );
                    ...
                    if (modelParts.length > 1) {
                        ...
                        const thumbRel = model.thumbnailUrl.replace(
                            '/api/uploads/',
                            ''
                        );
                        ...
                        if (
                            ...
                            if (
                                ...
                                console.log(
                                    `Consolidated thumb -> ${targetFolder}/...`
                                );
                                ...
                                if (oldThumbDir !== uploadDir) {
                                    try {
                                        if (
                                            fs.readdirSync(oldThumbDir)
                                                .length === 0
                                        ) {
                                            fs.rmdirSync(oldThumbDir);
                                            console.log(
                                                'Removed empty thumb dir'
                                            );
                                        }
                                    } catch (e) {
